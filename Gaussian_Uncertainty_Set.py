...
def evaluate_gaussian_knownV(num_samples, confidence_level, num_simulation, value_function, min_demand = 0, max_demand = 100, demand_mean_prior_mean = 50, demand_mean_prior_std = 15, true_demand_std = 25):
    ...
    return [(Methods.KNOWNV, np.mean(abs(knownV_ret_err)), np.mean(knownV_th), np.mean(knownV_ret_err<0), np.mean(knownV_ret), np.std(abs(knownV_ret_err)), np.std(knownV_th), np.mean(KnownV_nomianl_point,axis=0))]
